...
int main(int argc, char **argv) {
    ...
    node->declare_parameter("weighted_pose", weighted_pose);
    ...
    rclcpp::Subscription<geosacs_msgs::msg::WeightedPose>::SharedPtr weighted_pose_sub;

    rclcpp::Subscription<geometry_msgs::msg::PoseStamped>::SharedPtr twist_sub2;




    weighted_pose_sub = node->create_subscription<geosacs_msgs::msg::WeightedPose>(
        "/weighted_pose", 10,
        [&](const geosacs_msgs::msg::WeightedPose::SharedPtr msg) {
            if (weighted_pose) {
                RCLCPP_INFO_ONCE(node->get_logger(), "Weighted pose received");
    ...
    twist_sub2 = node->create_subscription<geometry_msgs::msg::PoseStamped>(
        "/commanded_pose", 1,
        [&](geometry_msgs::msg::PoseStamped::SharedPtr msg) {
            if (!weighted_pose) {
                RCLCPP_INFO_ONCE(node->get_logger(), "Commanded pose received");
                nextCommandedPose.pose = msg->pose;
                start = true;
            }
        });
