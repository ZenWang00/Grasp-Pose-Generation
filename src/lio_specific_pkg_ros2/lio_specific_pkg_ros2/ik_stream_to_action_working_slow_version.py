################# TWO VERSION EXIST: TOP IS THE MOST SMOOTHE ONE< SECOND IS THE INITIAL NOT MUCH SMOOTHED ONE #######################
...
#         # streaming state (for stream-ik)
#         self._last_cmd_deg = None
#         self._goal_handle = None
#         self._eps = 0.05  # degrees
#         self._sub = None
#         self._bridge = "core"
...
#         """One-shot helper for read/write modes."""
#         goal = ExecuteFunction.Goal()
#         goal.action = action
#         goal.bridge = bridge
...
#     # ----------------- STREAM FROM /panda_ik/output -----------------
#     def start_stream_ik(self, topic: str, eps_deg: float, bridge: str):
#         """Subscribe to /panda_ik/output (Float64MultiArray in radians) and stream to action in degrees."""
#         self._eps = float(eps_deg)
#         self._bridge = bridge
...
#             depth=10,
#         )
#         self._sub = self.create_subscription(Float64MultiArray, topic, self._ik_cb, qos)
#         self.get_logger().info(
#             f"Streaming IK from {topic} (radians) → {self._client._action_name} (degrees), eps={self._eps}°"
#         )
...
#             return
#         arm6_rad = data[:6]
#         arm6_deg = [math.degrees(x) for x in arm6_rad]

#         # print the converted degrees every time we evaluate (so you can see the exact numbers)
#         self.get_logger().info(
#             "IK target (deg): " + ", ".join(f"{v:.3f}" for v in arm6_deg)
#         )

#         # only send if changed beyond epsilon
#         if not _changed(arm6_deg, self._last_cmd_deg, self._eps):
#             return
...
# def build_arg_parser():
#     p = argparse.ArgumentParser(
#         description="Read/write or stream IK to robot via fp_core ExecuteFunction action"
#     )
#     p.add_argument('--server-name', default='/execute_function',
#                    help='Action server name (e.g., /execute_function or /robot/execute_function)')

#     # DO NOT require a subcommand; default to stream-ik.
#     sub = p.add_subparsers(dest='cmd')

#     # STREAM-IK (default)
#     s = sub.add_parser('stream-ik', help='Subscribe to /panda_ik/output and stream to action (deg out)')
#     s.add_argument('--topic', default='/panda_ik/output',
#                    help='IK output topic (Float64MultiArray in radians)')
#     s.add_argument('--bridge', default='core', help='Bridge name (default: core)')
#     s.add_argument('--eps', type=float, default=0.05,
#                    help='Min change (degrees) to trigger a new goal')
...
#     # WRITE (one-shot) — supports per-joint values via --joint-positions
#     w = sub.add_parser('write', help='Move joint(s) to position (one-shot)')
#     w.add_argument('--joints', type=int, nargs='+', required=True)
#     w.add_argument('--joint-position', type=float, required=False,
#                    help='Single target value applied to all listed joints (degrees)')
#     w.add_argument('--joint-positions', type=float, nargs='+', required=False,
#                    help='One target value per joint (degrees); length must match --joints')
#     w.add_argument('--bridge', default='core')

#     # default behavior when you run with no subcommand:
#     p.set_defaults(
#         cmd='stream-ik',
#         topic='/panda_ik/output',
#         bridge='core',
...
#     if args.cmd == 'stream-ik':
#         node.start_stream_ik(topic=args.topic, eps_deg=args.eps, bridge=args.bridge)
#         rclpy.spin(node)
#         node.destroy_node()
#         return 0
...
#         # log the outgoing values (assumed degrees)
#         if isinstance(joint_position, list):
#             node.get_logger().info(
#                 f"Write move_joints → joints={joints}, joint_position(deg)={[f'{v:.3f}' for v in joint_position]}"
#             )
